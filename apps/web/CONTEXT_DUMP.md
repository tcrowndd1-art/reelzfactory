...
### Google Gemini TTS
- ALL_VOICES (25개) 임포트 → 성별 필터링 적용 (남12/여11)
- 하드코딩된 GOOGLE_VOICES_MALE/FEMALE 삭제
- 스타일 3개 버튼(인지적갭/팩폭형/CEO형) 삭제 → stylePrompt textarea 추가
- Pro/Flash TTS 모델 선택 UI 추가 + 가격 안내
- voice-preview 모델 순서 Pro 우선으로 수정 (flash→pro를 pro→flash로)
- 속도 프리셋 5개 버튼 추가 (느리게/보통/빠르게/더빠르게/아주빠르게)
- 서버 pacingNote 자동 변환 활용 (옵션 α)
- Voice Style 카드 pace 표시 삭제
- 미리듣기에 speed 전달 추가

### OpenAI TTS
- 성별 필터 적용 (filteredOpenAIVoices 사용)
- 성별 아이콘(👨/👩) 추가 + "총 6개 보이스" 안내
- Speed 슬라이더 OpenAI 전용으로 분리 (Google에서 제거)
- 미리듣기 fetch에 speed: voiceSpeed 추가
- OPENAI_NATIVE_VOICES 체크 추가 (generate-voice + voice-preview 양쪽)
  → echo 선택 시 nova로 빠지던 매핑 버그 수정
- .env에 OPENAI_API_KEY 추가 (수동)
- process.env.OPENAI_API_KEY fallback 추가 (voice-preview)

### Clone (Fish Audio)
- 미리듣기 버튼 추가 (localStorage에서 fishApiKey/fishModelId 읽기)
- reelzfactory_settings JSON 구조에 맞게 키 읽기 수정
- Settings에 Fish Model ID 입력란 추가
- prosody.speed 전달 확인 (voice-preview + generate-voice 양쪽 이미 적용)
- Clone 미리듣기 fetch에 speed: voiceSpeed 추가

### 공통 UI 수정
- isSpeedControllable / speedNote 변수 삭제
- Footer activeVoiceName fallback 제거 (Leda 노출 버그 수정)
- toFixed(2) 통일
- 슬라이더 tick 마커 색/크기 축소
- Category 탭 완전 삭제
- Gender 섹션 별도 블록 분리 (Google Style 탭 전용)

### 채널 설정 (channels/[id]/page.tsx)
- ALL_VOICES 임포트 + 성별 필터링
- stylePrompt textarea + 적용 버튼 (Gemini/OpenAI)
- Pro/Flash 모델 선택 + 가격 안내
- 씬별 감정매핑 블록 삭제 (Clone)
- JSX 닫기 태그 누락 수정 (1018-1021행)

### 대본/카테고리
- prompts.ts 씬 수 강제: 30초→6-10씬, 15초→3-5씬, 60초→12씬
- 8개 카테고리 상세 지침서 작성 (health, psychology, invest 등)
- ttsStylePrompts.ts speed 기본값 0.95→1.0 변경

### 미리듣기 로그 강화
- voice-preview 로그에 Model/Style 필드 추가

### 보안
- .env 파일 터미널 노출 사고 → 절대 터미널로 .env 건드리지 않기 가이드라인 수립


## 2026-04-04 세션 업데이트

### 완료된 작업
- [A1] channels 테이블에 script_preset JSONB 컬럼 추가 (ALTER TABLE)
- [A2] DEFAULT_SCRIPT_PRESET 정의 (store.ts 89행, as const 타입)
- [B1] constants/engineMap.ts 신규 생성 (5개 모델: claude-sonnet-4.6, gemini-2.5-pro, gemini-2.5-flash, gpt-4o, kimi-k2.5)
- [B2] route.ts 엔진 라우팅 구현 (resolveEngine → 동적 modelId, maxTokens)
- [C1] Script 탭 UI 구현 (채널 설정 페이지, 엔진 선택 + customPrompt + validator 토글)
- [C2] saveChannel에 script_preset 포함
- [C3] create/page.tsx에서 scriptEngine + customPrompt를 API에 전달
- [D1] Fish Audio 태그 버그 — 이전 세션에서 이미 해결 확인
- [D2] 카테고리 자동 감지 — 이미 ch.niche 기반으로 동작 확인
- 이미지 스타일 가이드 대본 프롬프트 오염 수정 (route.ts 186-194행 제거)
- topic 미전달 버그 수정 (prompts.ts에 input.topic 추가)
- customPrompt 프롬프트 주입 연결 (route.ts finalPrompt 변수)
- allScenes.push 누락 수정 (프리뷰 미표시 원인)
- referenceImages → resolvedCharRef 연결 (generate-images/route.ts)
- StoryboardStep/VoiceStep/ScriptStep segments 정규화 (롱폼 호환)
- UserSettings.channel 타입에 preset 필드 추가 (typecheck 통과)

### 엔진 매핑 (OpenRouter)
| Key | Model ID | Max Tokens | 용도 |
|-----|----------|------------|------|
| claude-sonnet-4.6 | anthropic/claude-sonnet-4.6 | 16384 | 숏폼/롱폼 기본 |
| gemini-2.5-pro | google/gemini-2.5-pro | 16384 | 고급 추론 |
| gemini-2.5-flash | google/gemini-2.5-flash | 16384 | 저비용 숏폼 |
| gpt-4o | openai/gpt-4o | 8192 | 범용 |
| kimi-k2.5 | moonshotai/kimi-k2.5 | 16384 | 데이터 크롤링/요약 |

### Content Strategy Matrix v2.0
| 코드 | 전략명 | 시각 | 톤 |
|------|--------|------|-----|
| A1 | SIMULATION: MONKEY (경제) | 2D 카툰 원숭이 | 냉소적 해라체 |
| A2 | SIMULATION: MONKEY (심리) | A1 공유 | 냉소적 각성 |
| B1 | SCIENTIFIC: GUIDE | 실사+배경영상 레이아웃2 | 공감적 존칭 |
| B2 | NEWS-BREAK: TRUST | 뉴스캡처+배경영상 레이아웃2 | 단정적 보도체 |
| C1 | DRAMA: DIALOGUE | 애니/의인화 캐릭터 | 구어체 풍자 |

### 미구현 / 잔여 작업
- [ ] 탭 기본값 voice → script (channels/[id]/page.tsx 116행)
- [ ] UI 라벨 "프롬프트 접두사" → "Image Style Guide" (572행)
- [ ] Chain-Link 이미지→비디오 파이프라인 (신규)
  - generate-video/route.ts 생성 필요
  - StoryboardStep Video 버튼 → start_image(현재씬) + end_image(다음씬) + video_prompt
  - 대본 생성 시 video_prompt 필드 추가 (숏폼 템플릿에 미포함)
  - Kling/Veo API 연동
- [ ] knowledge_packs에 strategy_code + language 조합 톤 오버레이
- [ ] B1/B2 레이아웃2 (배경 영상 + 오버레이) Remotion 컴포지션
- [ ] C1 멀티 보이스 TTS 매핑
- [ ] YouTube Analytics 연동 (Phase 1)
- [ ] 스마트 학습 시스템 (Phase 2-3)

### 프롬프트 플로우 (현재)
사용자 주제 → buildUserPrompt(topic, mode, language, category...)
  + categoryOverlay (knowledge_packs에서 로드)
  + customPrompt (채널 Script 탭에서 로드)
  → finalPrompt → OpenRouter API (resolveEngine으로 모델 선택)

### 이미지 플로우 (현재)
씬별 imagePrompt + promptPrefix(채널 Image 탭) + referenceImages[0] → resolvedCharRef
  → fal.ai (Kontext 모델 시 레퍼런스 이미지 전달)

### 알려진 이슈
- Supabase productions 테이블 GET 406 에러 (프론트 저장 시)
- Validator가 마침표를 경고하지만 자동 수정 미구현
- 롱폼 Remotion 16:9 렌더링 미검증
=========================================================

## 2026-03-24 업데이트
- 블록 1~19 전부 완료 (prompt_templates 13행 + knowledge_packs 5행 + formatMatrix.ts 1파일)
- Supabase 테이블 추가 완료: prompt_templates, knowledge_packs, channel_knowledge, purchases, pipeline_logs
- users 테이블 과금 필드 추가 (plan, stripe, limits)
- 가격 전략 확정: Option B (사용자 API 키 + 구독제)
- Sidebar에 Channels 링크 추가, Dashboard 프로젝트 클릭 이동 추가
# ReelzFactory — 프로젝트 컨텍스트 (2026-03-23 업데이트)

## 1. 프로젝트 개요
- YouTube Shorts/Longform 자동 생성 파이프라인
- Next.js 15 + Remotion + Supabase + AI APIs
- 경로: C:\Dev\reelzfactory
- 웹앱: apps/web (포트 3000)
- 렌더링: remotion/
...
### Phase 0: 기반 정리
- [x] OneDrive → C:\Dev\reelzfactory 로컬 이동
- [x] .env 통합 (루트 .env만 사용, dotenv로 로드)
- [x] God Component 분리 (create/[id]/page.tsx 877→170줄)
- [x] 6 커스텀 훅, 7 컴포넌트, 타입·상수 분리
- [x] Next.js 15 호환 작업

### Phase 1: Supabase + Channel (Block A+B)
- [x] Supabase 연결 (lib/supabase.ts)
- [x] store.ts → Supabase 전환 (async CRUD)
- [x] 호출부 await 전환 (page.tsx, projects, 3개 훅)
- [x] Channel Manager UI (app/channels/page.tsx, app/channels/[id]/page.tsx)
- [x] 채널 프리셋 5탭 (Voice, Image, Subtitle, Render, YouTube)
- [x] 프로젝트 생성 시 채널 드롭다운 + 프리셋 자동 적용
- [x] Sidebar에 Channels 링크 추가
- [x] Dashboard Recent Projects 클릭 이동 추가

### Phase 1.5: DB 스키마 확장
- [x] pipeline_logs 테이블 생성
- [x] knowledge_packs 테이블 생성
- [x] channel_knowledge 테이블 생성
- [x] purchases 테이블 생성
- [x] prompt_templates 테이블 생성
- [x] users 테이블 과금 필드 추가 (plan, stripe, limits)
- [x] 전체 인덱스 + RLS + 트리거 설정

## 3. Supabase 테이블 현황 (전체)

| 테이블 | 용도 |
|--------|------|
| users | 사용자 + 과금 (plan, stripe_*, limits) |
| channels | 채널 + 4종 프리셋 (JSONB) |
| productions | 프로젝트 (preset snapshot, checkpoint) |
| scenes | 씬 단위 데이터 |
| cost_logs | API 비용 추적 |
| pipeline_logs | 파이프라인 단계별 로그 |
| knowledge_packs | 카테고리별 팩트/데이터 (판매용) |
| channel_knowledge | 채널별 커스텀 자료 |
| prompt_templates | LLM 프롬프트 블록 (비공개) |
| purchases | 구매 기록 |
| admin_users | 관리자 |
| analytics_daily | 일별 성과 |
| api_keys | 사용자 API 키 |
| comments | 댓글 |
| feature_usage | 기능 사용량 |
| ref_materials | 참고 자료 |
| schedules | 예약 업로드 |
| series | 시리즈 |
| spy_targets | 벤치마킹 대상 |
...
## 4. prompt_templates 블록 현황 (19개)

| # | 블록 | block_type | category | mode | name | 상태 |
|---|------|-----------|----------|------|------|------|
| 1 | 공통 코어 | core | common | all | common_core | ✅ DB |
| 2 | 숏폼 코어 | core | common | shorts | shorts_core | ✅ DB |
| 3 | 롱폼 코어 | core | common | longform | longform_core | ✅ DB |
| 4 | 숏폼 어댑터 — 우화 | adapter | common | shorts | adapter_allegory | ⬜ 대기 |
| 5 | 숏폼 어댑터 — 팩트폭격 | adapter | common | shorts | adapter_factbomb | ⬜ 대기 |
| 6 | 숏폼 어댑터 — 비교리뷰 | adapter | common | shorts | adapter_compare | ⬜ 대기 |
| 7 | 숏폼 어댑터 — POV | adapter | common | shorts | adapter_pov | ⬜ 대기 |
| 8 | 숏폼 어댑터 — 미스터리 | adapter | common | shorts | adapter_mystery | ⬜ 대기 |
| 9 | 롱폼 소스 — Quick | adapter | common | longform | adapter_quick | ⬜ 대기 |
| 10 | 롱폼 소스 — Benchmark | adapter | common | longform | adapter_benchmark | ⬜ 대기 |
| 11 | 롱폼 소스 — Reference | adapter | common | longform | adapter_reference | ⬜ 대기 |
| 12 | 카테고리 — 건강 | overlay | health | all | overlay_health | ⬜ 대기 |
| 13 | 카테고리 — 심리 | overlay | psychology | all | overlay_psychology | ⬜ 대기 |
| 14 | 카테고리 — 투자 | overlay | invest | all | overlay_invest | ⬜ 대기 |
| 15 | 카테고리 — 경제 | overlay | economy | all | overlay_economy | ⬜ 대기 |
| 16 | 카테고리 — 핫이슈 | overlay | hotissue | all | overlay_hotissue | ⬜ 대기 |
| 17 | 포맷×카테고리 매트릭스 | code | — | — | constants/formatMatrix.ts | ✅ 확정 |
| 18 | Validator 롱폼 | validator | common | longform | validator_longform | ⬜ 대기 |
| 19 | Validator 숏폼 | validator | common | shorts | validator_shorts | ⬜ 대기 |
...
| Creator | $24 | 5 | 150 |
| Agency | $59 | 15 | 무제한 |
| Enterprise | $149 | 무제한 | 무제한 |

- 사용자가 본인 API 키 입력 (David API 비용 $0)
- 추가 수익: Knowledge Pack 마켓플레이스, 프리미엄 프리셋 판매

## 6. 영상 1편 API 비용 (사용자 부담)

| 조합 | 비용/편 |
|------|---------|
| Premium (DALL-E 3 + ElevenLabs) | $0.321 |
| Standard (GPT Image Mini + ElevenLabs) | $0.176 |
| Budget (GPT Image Mini + OpenAI TTS) | $0.068 |
...
| OPENAI_API_KEY | ✅ |
| ANTHROPIC_API_KEY | ✅ |
| ELEVENLABS_API_KEY | 확인 필요 |
| TELEGRAM_BOT_TOKEN | ✅ |
| TELEGRAM_CHAT_ID | ✅ |
| YOUTUBE_CLIENT_ID | 확인 필요 |
| YOUTUBE_CLIENT_SECRET | 확인 필요 |

## 8. 파일 구조 (주요)

Copy
C:\Dev\reelzfactory
├── .env (통합 환경변수) ├── apps/web/ │ ├── app/ │ │ ├── page.tsx (Dashboard) │ │ ├── create/page.tsx (프로젝트 생성 + 채널 드롭다운) │ │ ├── create/[id]/page.tsx (파이프라인) │ │ ├── channels/page.tsx (채널 목록) │ │ ├── channels/[id]/page.tsx (채널 설정 5탭) │ │ ├── projects/page.tsx (프로젝트 목록) │ │ ├── settings/page.tsx (설정) │ │ └── onboarding/page.tsx (온보딩) │ ├── components/ │ │ ├── Sidebar.tsx (NAV + Channels 링크) │ │ └── AutopilotBar.tsx │ ├── hooks/pipeline/ │ │ ├── useAutopilot.ts │ │ ├── useImageGeneration.ts │ │ ├── useScriptGeneration.ts │ │ ├── useVoiceGeneration.ts │ │ └── useProjectLoader.ts │ ├── lib/ │ │ ├── supabase.ts │ │ ├── store.ts (Supabase CRUD) │ │ ├── notify.ts │ │ └── pipelineCheckpoint.ts │ ├── constants/ │ │ └── presets.ts (DEFAULT_VOICE/IMAGE/SUBTITLE/RENDER_PRESET) │ └── next.config.ts ├── remotion/ │ ├── public/fonts/PretendardVariable.woff2 (6.5MB) │ └── src/ └── CONTEXT.md (이 파일)
...
| 순서 | 블록 | 내용 | 상태 |
|------|------|------|------|
| 1 | formatMatrix.ts | 포맷×카테고리 허용 매트릭스 상수 | ⬜ 다음 |
| 2 | 블록 4~19 INSERT | prompt_templates + knowledge_packs 데이터 | ⬜ 진행중 |
| 3 | Block C | 자막 → Remotion 연동 | ⬜ |
| 4 | Block D | Remotion 이펙트 (transitions + motions) | ⬜ |
| 5 | Block E | 오토파일럿 안정화 | ⬜ |
| 6 | Block F | 폰트/스키마 정리 | ⬜ |
| 7 | Block G | generate-script API 리팩터링 (블록 조합) | ⬜ |
| 8 | Phase 3 | Stripe 연동, SaaS 배포 | ⬜ |

## 10. 대본 시스템 아키텍처

[사용자 선택] 모드(숏폼/롱폼) + 카테고리 + 포맷(숏폼만) + 주제

[시스템 조합] 공통코어 + 모드코어 + 포맷어댑터 + 카테고리오버레이 → LLM에 전달 → 대본 JSON 생성 → Validator 검증 → (실패 시) REVISE 존 자동 재작성

[저장 위치] 프롬프트 블록: prompt_templates (Supabase) 팩트/데이터: knowledge_packs (Supabase, 마켓플레이스 판매용) 채널 커스텀 자료: channel_knowledge (Supabase) 매트릭스: constants/formatMatrix.ts (코드)
...
-----------------------------
Context 전체 업데이트 (2026-03-23 현재)
완료된 작업 (Phase 0)
인프라: OneDrive→로컬(C:\Dev\reelzfactory) 이동 완료. .env 통합(루트 .env만 사용, .env.local 삭제, next.config.ts에서 dotenv로 로드). 중복 package-lock.json 삭제. .vscode/settings.json 생성(워크스페이스 TS SDK). package.json에 typecheck/prebuild 스크립트 추가.

God Component 분리 완료: create/[id]/page.tsx 877줄→170줄 오케스트레이터. 6개 커스텀 훅(hooks/pipeline/ — useProjectLoader, useScriptGeneration, useVoiceGeneration, useImageGeneration, useRenderPipeline, useYoutubeUpload), 7개 컴포넌트(components/pipeline/ — PipelineSteps, ScriptStep, VoiceStep, VoiceSettingsModal, StoryboardStep, RenderStep, UploadStep), 타입 정의(types/pipeline.ts), 상수(constants/voiceConfig.ts).

버그 수정: Projects 리스트 클릭 버그(onClick + router.push 추가). Next.js 15 params Promise 호환(knowledge/[niche]/route.ts). render-video/route.ts 리턴타입 Promise. generate-voice/route.ts openaiKey 타입 추가. create/page.tsx 암시적 any 타입 8건 수정. page.tsx 중복 fontSize 제거. projects/page.tsx scriptSource→source. ttsStylePrompts.ts 인덱싱 as any 캐스팅.

오토파일럿 기초: lib/notify.ts(콘솔+localStorage, Telegram 인터페이스 준비), lib/pipelineCheckpoint.ts(체크포인트 CRUD), hooks/pipeline/useAutopilot.ts(상태머신), components/pipeline/AutopilotBar.tsx(UI). pipelineCheckpoint.ts에 잔여 타이포(und, efined) 수정 완료. 오토파일럿 버튼 UI 확인됨.

store.ts 업그레이드: Project 타입을 ProjectCore + ProjectExtended로 분리. migrateProject 함수로 레거시 프로젝트 호환. category, language 필드 추가.

현재 파일 구조
C:\Dev\reelzfactory\apps\web\
├── app/
│   ├── page.tsx (대시보드)
│   ├── create/
│   │   ├── page.tsx (프로젝트 생성)
│   │   └── [id]/page.tsx (파이프라인 오케스트레이터 ~170줄)
...
│       ├── analyze-benchmark/route.ts
│       ├── knowledge/[niche]/route.ts
│       └── auth/youtube/callback/route.ts
├── components/pipeline/
│   ├── PipelineSteps.tsx, ScriptStep.tsx, VoiceStep.tsx
│   ├── VoiceSettingsModal.tsx, StoryboardStep.tsx
│   ├── RenderStep.tsx, UploadStep.tsx, AutopilotBar.tsx
├── hooks/pipeline/
│   ├── useProjectLoader.ts, useScriptGeneration.ts
│   ├── useVoiceGeneration.ts, useImageGeneration.ts
│   ├── useRenderPipeline.ts, useYoutubeUpload.ts
│   └── useAutopilot.ts
├── lib/
│   ├── store.ts (migrateProject 포함)
│   ├── notify.ts, pipelineCheckpoint.ts
│   ├── prompts.ts, knowledge.ts
│   └── ttsStylePrompts.ts
├── types/pipeline.ts
├── constants/voiceConfig.ts
└── next.config.ts (dotenv 로드)
환경
루트 .env: C:\Dev\reelzfactory\.env (OPENAI_API_KEY, GEMINI_API_KEY, FAL_KEY, YOUTUBE_CLIENT_ID, YOUTUBE_CLIENT_SECRET, YOUTUBE_REDIRECT_URI, YOUTUBE_REFRESH_TOKEN, OPENROUTER_API_KEY 등). 서버: cd C:\Dev\reelzfactory\apps\web && npm run dev (localhost:3000). 테스트 프로젝트 ID: 1774225587568 (전체 파이프라인 검증 완료).
--------------------
최종 실행 로드맵2303
═══════════════════════════════════════════════════════
 Phase 0: 기반 정비 (오토파일럿 전 필수)
═══════════════════════════════════════════════════════

 0-1. Projects 목록 클릭 버그 수정
 0-2. create/[id]/page.tsx → 커스텀 훅 5개 + 탭 컴포넌트 5개 분리
 0-3. PipelineCheckpoint 시스템 구축 (store.ts 확장)
 0-4. .env 단일화 (루트 1개로 통일)
 0-5. 포트 3099 동적 할당 (render.mjs)
 0-6. 이미지 null → placeholder 삽입 로직 (Scene.tsx)

═══════════════════════════════════════════════════════
 Phase 1: 오토파일럿 코어
═══════════════════════════════════════════════════════

 1-1. useAutopilot.ts 상태머신 설계
      idle → voice → image → render → upload → done
      각 상태에서 error → 정책별 분기 (재시도/스킵/중단)
 1-2. 단계별 타임아웃 설정
      Voice: 씬당 30초, 전체 5분
      Image: 씬당 60초, 전체 10분
      Render: 전체 5분
      Upload: 전체 3분
 1-3. notify.ts 알림 인터페이스 구축
 1-4. voice-mapping.json 분리
 1-5. Upload 탭 UI (제목/태그/공개설정/썸네일)
 1-6. YouTube 토큰 만료 감지 + 재인증 플로우
 1-7. AutopilotBar.tsx (진행률 + 상태 실시간 표시)
...
═══════════════════════════════════════════════════════
 ...
 2-1. 텔레그램 봇 실연동
 2-2. 비용 트래킹 (각 API 호출당 비용 기록)
 2-3. Longform 16:9 렌더링 분기
 2-4. 프롬프트 개선 (훅킹/CTA/Benchmark 정교화)
 2-5. 에러 대시보드 (프로젝트별 에러 히스토리)
======================================
...
### ?? ??
- v2.0 ? v3.2 ?? (13 SECTION: ???, 4-GATE, ?? ????, ?? ??, ??? ??, ??, ??, ?? ??, ?? ??, ?? ??, CEO ???, ?? ??, ?? ??)
- ?? ??? ?? (???? ??? ?? ??)

### ??? #1 ??
- ??: Lost in the Middle + RULE_REMINDER ??? + ??? ?? (20?)
- ??: common_core v3.0 ? v4.0 (?? 7? + ?? ??) + scriptValidator.ts (???)
- ??: ?? ??? (shorts_core ?? ? ??)

### Remotion Skills ??
- ??: remotion/.agents/skills/remotion-best-practices/rules/ (37? ??)
- ?? 13? ??: animations, transitions, subtitles, sequencing, images, audio, fonts, timing, text-animations, assets, sound-effects, videos, charts
- Phase 3?? ?? ??? ?? ??

### ?? ?? ?? ?
1. ?? ?? v3.2 + ? CONTEXT_DUMP.md ????
2. shorts_core SELECT ?? + common_core v4.0 ???? ?? ??

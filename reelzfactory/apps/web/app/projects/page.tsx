    ...
    <div style={{ padding: 32, maxWidth: 1200, margin: "0 auto" }} className="animate-fadeIn">
      ...
      {projects.length === 0 ? (
        <div style={{ background: "#111827", border: "1px solid #1e293b", borderRadius: 12, padding: 64, textAlign: "center" }}>
          <p style={{ fontSize: 36, marginBottom: 16 }}>{"\u{1F4C1}"}</p>
            ...
            <div key={p.id} style={{
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: "#64748b", marginTop: 4 }}>
                  {p.mode === "longform" ? "Long-form" : "Shorts"} · {p.scriptSource} · {new Date(p.createdAt).toLocaleDateString()}
                </p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                <span style={{ fontSize: 12, fontFamily: "monospace", color: "#94a3b8" }}>${(p.estimatedCost || 0).toFixed(2)}</span>
                <span style={{
